...
class Payment(models.Model):
    ...
    supabase_proof_of_payment_url = models.URLField(blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        # Upload proof_of_payment to Supabase if set and supabase_proof_of_payment_url is empty or new instance
        if self.proof_of_payment and (not self.supabase_proof_of_payment_url or not self.pk):
            storage_service = SupabaseStorageService()
            upload_result = storage_service.upload_file(self.proof_of_payment, folder='payments')
            if upload_result.get('success'):
                self.supabase_proof_of_payment_url = upload_result.get('public_url')
...
class Deposit(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.proof_of_payment and (not self.supabase_proof_of_payment_url or not self.pk):
            storage_service = SupabaseStorageService()
            upload_result = storage_service.upload_file(self.proof_of_payment, folder='deposits')
            if upload_result.get('success'):
                self.supabase_proof_of_payment_url = upload_result.get('public_url')
        super().save(*args, **kwargs)    
...
class MaintenancePayment(models.Model):
    ...
    supabase_proof_screenshot_url = models.URLField(blank=True, null=True)
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default=PENDING)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        # Upload proof_screenshot to Supabase if set and supabase_proof_screenshot_url is empty or new instance
        if self.proof_screenshot and (not self.supabase_proof_screenshot_url or not self.pk):
            storage_service = SupabaseStorageService()
            upload_result = storage_service.upload_file(self.proof_screenshot, folder='maintenance_fees')
            if upload_result.get('success'):
                self.supabase_proof_screenshot_url = upload_result.get('public_url')
        super().save(*args, **kwargs)
