...
class PaymentAdmin(admin.ModelAdmin):
    ...
    ordering = ('-created_at',)
    readonly_fields = ('created_at', 'updated_at', 'proof_of_payment_preview',)
    actions = ['mark_as_completed', 'mark_as_delivered']

    def proof_of_payment_preview(self, obj):
        if obj.supabase_proof_of_payment_url:
            return format_html('<img src="{}" style="max-height: 100px; max-width: 100px;" />', obj.supabase_proof_of_payment_url)
        elif obj.proof_of_payment:
            return format_html('<img src="{}" style="max-height: 100px; max-width: 100px;" />', obj.proof_of_payment.url)
        return "-"
...
class DepositAdmin(admin.ModelAdmin):
    list_display = ('transaction_id', 'user', 'amount', 'payment_method', 'proof_of_payment_preview', 'colored_status', 'created_at')
    list_filter = ('status', 'payment_method', 'created_at')
...
class MaintenancePaymentAdmin(admin.ModelAdmin):
    list_display = ('user', 'full_name', 'phone_number', 'amount', 'status', 'created_at', 'proof_screenshot_preview')
    list_filter = ('status', 'created_at')
    search_fields = ('user__username', 'full_name', 'phone_number')
    ordering = ('-created_at',)
    readonly_fields = ('proof_screenshot_preview',)

    def proof_screenshot_preview(self, obj):
        if obj.supabase_proof_screenshot_url:
            return format_html('<img src="{}" style="max-height: 100px; max-width: 100px;" />', obj.supabase_proof_screenshot_url)
        elif obj.proof_screenshot:
            return format_html('<img src="{}" style="max-height: 100px; max-width: 100px;" />', obj.proof_screenshot.url)
        return "-"
    proof_screenshot_preview.short_description = 'Proof Screenshot Preview'
