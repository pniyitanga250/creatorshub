...
class ActivationFeeProof(models.Model):
    ...
    supabase_proof_image_url = models.URLField(blank=True, null=True)
    timestamp = models.DateTimeField(auto_now_add=True, db_index=True)
    ...
    def save(self, *args, **kwargs):
        if self.proof_image and (not self.supabase_proof_image_url or not self.pk):
            from services.storage_service import SupabaseStorageService
            storage_service = SupabaseStorageService()
            upload_result = storage_service.upload_file(self.proof_image, folder='activation_fee_proofs')
            if upload_result.get('success'):
                self.supabase_proof_image_url = upload_result.get('public_url')
        super().save(*args, **kwargs)
